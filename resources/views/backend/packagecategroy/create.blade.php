...
@section('title', 'Create Package Categroy')

@section('content')
    <x-forms.post :action="route('packagecategroy.store')">
        <x-backend.card>
            <x-slot name="header">
                Create Package Categroy
            </x-slot>

            <x-slot name="headerActions">
                <x-utils.link class="card-header-action" :href="route('packagecategroy.index')" :text="__('Cancel')" />
            </x-slot>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-10">
                            <input type="number" name="price" class="form-control" placeholder="Enter Price"
                                value="{{ old('price') }}" maxlength="100" required />
                        </div>
                    </div>
                    <!--form-group-->
            ...
            <x-slot name="footer">
                <button class="btn btn-sm btn-primary float-right" type="submit">Create Package Categroy</button>
            </x-slot>
        </x-backend.card>
    </x-forms.post>
@endsection
