            ...
            <x-slot name="body">
                <div x-data="{ userType: '{{ $role->type }}' }">
                        ...
                        <div class="col-md-10">
                            <select name="type" class="form-control" required x-on:change="userType = $event.target.value">
                                <option value="{{ $model::TYPE_USER }}"
                                    {{ $role->type === $model::TYPE_USER ? 'selected' : '' }}>@lang('User')</option>
                                <option value="{{ $model::TYPE_ADMIN }}"
                                    {{ $role->type === $model::TYPE_ADMIN ? 'selected' : '' }}>@lang('Administrator')</option>
                            </select>
                        </div>
                    </div>
                    <!--form-group-->
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-10">
                            <input type="text" name="title" class="form-control" placeholder="{{ __('Name') }}"
                                value="{{ old('title') ?? $role->title }}" maxlength="100" required />
                        </div>
                    </div>
                    <!--form-group-->
