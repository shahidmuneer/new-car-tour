...
;(function($, window, undefined){ 
  ...
  $.Calendario.prototype = {
    ...
    _generateTemplate : function(firstRun, callback) {
      ...
      this.$el.find('.fc-emptydate').parent().removeClass('fc-future').css({'background':'transparent', 'cursor':'default'});
      ...
      for (var i = 0; i < 7; i++){ // this loop is for weeks (rows)
        for (var j = 0; j <= 6; j++) { // this loop is for weekdays (cells)
          ...
          if(content !== '') {
              ...
              if (content.match(/class="race"/)) {
                  cellClasses += 'race ';
              } else if (content.match(/class="disabled"/)) {
                  cellClasses += 'not-available ';
              } else if (content.match(/class="(closed|partial)"/)) {
                  cellClasses += 'partially-available ';
              } 
          }
